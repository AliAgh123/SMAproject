...
def cmd_benchmark(args: argparse.Namespace) -> int:
    ...
    from sma_chess.analytics.benchmarking import run_benchmark

    graph = _load_analyzed_graph(args.graph)
    df = run_benchmark(graph, node_sizes=args.node_sizes)
    ...
    if args.figure:
        ...
        axes[0].plot(df["Nodes"], df["Time_Custom_DFS"], marker="o", label="Custom DFS")
        axes[0].plot(df["Nodes"], df["Time_NX_DFS"], marker="s", label="NetworkX DFS")
        axes[0].plot(df["Nodes"], df["Time_NX_BFS"], marker="^", label="NetworkX BFS")
        axes[0].plot(df["Nodes"], df["Time_Dijkstra"], marker="D", label="Dijkstra")
        ...
        axes[1].plot(df["Nodes"], df["Score_Optimal"], marker="D", label="Dijkstra")
        axes[1].plot(df["Nodes"], df["Score_Custom"], marker="o", label="Custom DFS")
        axes[1].plot(df["Nodes"], df["Score_Random"], marker="x", label="Random")
        axes[1].set_title("Path Score")
...
def cmd_visualize(args: argparse.Namespace) -> int:
    ...
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    from sma_chess.visualization import create_interactive_graph, create_interactive_path_graph
    ...
    if args.path_view:
        ...
        output_path = create_interactive_path_graph(graph, path, path_output)
...
def cmd_stockfish(args: argparse.Namespace) -> int:
    ...
    _print_json({"spearman_rho": rho, "p_value": pval, "ndcg": ndcg})
    return 0
